if (typeof add_javascript != 'undefined')
	add_javascript(get_script_path('StatusUI_Top.js')+"animation.js");
function StatusUI_Top(manager, margin) {
	...
	this.update_status_control = function(c, status) {
		...
		if (status.no_icon)
			...
			s = "<img src='"+common_images_url+"error.png' hspace=2 style='vertical-align:bottom;margin-bottom:2px'/>";
		else if (status.type == Status_TYPE_WARNING)
			s = "<img src='"+common_images_url+"warning.png' hspace=2 style='vertical-align:bottom;margin-bottom:2px'/>";
		else if (status.type == Status_TYPE_PROCESSING)
			s = "<img src='"+common_images_url+"loading.gif' hspace=2 style='vertical-align:bottom;margin-bottom:2px'/>";
		else
			s = "<img src='"+common_images_url+"info.png' hspace=2 style='vertical-align:bottom;margin-bottom:2px'/>";
		s += status.message;
		c.innerHTML = s;
		if (status.actions != null)
			for (var i = 0; i < status.actions.length; ++i) {
				...
				if (a.action == "close") {
					...
					img.src = common_images_url+"close_10.png";
					img.hspace=1;
